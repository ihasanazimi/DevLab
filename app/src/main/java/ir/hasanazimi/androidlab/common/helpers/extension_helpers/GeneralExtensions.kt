...
private fun String.convertPascalCaseToCamelCase():String{
    return this.replaceFirst(this.substring(0,1), this.substring(0, 1).lowercase(getDefault()))
}
...
inline fun <reified T> Gson.fromJsonOrNull(json: String?): T? {
    return try {
        if (json != null) fromJson(json, T::class.java) else null
    } catch (e: Exception) {
...
fun getPriceFormat(price: String?): String {
    ...
    return try {
        val doubleValue = price.toDouble()
        if (doubleValue % 1 != 0.0) {
            DecimalFormat("#,##0.00").format(doubleValue)
        } else {
            NumberFormat.getIntegerInstance(Locale.US).format(doubleValue.toLong())
        }
    } catch (e: Exception) {
        price
    }
...
fun String.getNormalizePhoneNumber(): String {
